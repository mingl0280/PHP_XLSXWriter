...
class XLSXWriter
{
    ...
    protected function initializeSheet($sheet_name)
    {
        ...
        $sheet->file_writer->write(    '<col collapsed="false" hidden="false" max="1025" min="1" style="0" width="20" bestFit="1" customWidth="1"/>');
        $sheet->file_writer->write(  '</cols>');
    ...
    protected function styleFontIndexes()
    {
        ...
        foreach($this->cell_styles as $i=>$cell_style_string)
        {
            ...
            if (isset($style['border']) && is_string($style['border']))
            {
                ...
                $style_indexes[$i]['border_idx'] = self::add_to_list_get_index($borders, implode(",", $border_input) );
            }
        ...
        foreach($borders as $border) {
            if (!empty($border)) { //fonts have an empty placeholder in the array to offset the static xml entry above
                ...
                foreach($pieces as $piece)
                {
                    if (strpos($piece,":"))
                    {
                        $wStyBorder=explode(":",$piece);
                        array_push($borderArray, $wStyBorder[0]);
                        $borderStyleArray[$wStyBorder[0]] = $wStyBorder[1];
                    }else
                    {
                        array_push($borderArray, $piece);
                        $borderStyleArray[$piece] = 'thin';
                    }
                }

                $file->write('<border diagonalDown="false" diagonalUp="false">');
                $file->write(  '<left'.(in_array('left',$borderArray) ? " style=\"".$borderStyleArray['left']."\"" : '').'/>');
                $file->write(  '<right'.(in_array('right',$borderArray) ? " style=\"".$borderStyleArray['right']."\"" : '').'/>');
                $file->write(  '<top'.(in_array('top',$borderArray) ? " style=\"".$borderStyleArray['top']."\"" : '').'/>');
                $file->write(  '<bottom'.(in_array('bottom',$borderArray) ? " style=\"".$borderStyleArray['bottom']."\"" : '').'/>');
                $file->write(  '<diagonal/>');
                $file->write('</border>');
            }
        }
